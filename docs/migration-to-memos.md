...
2026-09-19 Memos 已补齐 Go 回调、后台生命周期、三驱动队列、管理员设置、回执与旧 SQLite 导入工具，并归档本项目开通、需求及 Task 文档。具体测试事实以 Memos 主任务为准；本仓库未改 Python 运行代码，生产切换事实留在私有运维记录。切换须先完成验证与私有备份，停止旧消费者后转交内置模块，不能让两个消费者同时处理。旧仓库是否归档后续另行决定。
